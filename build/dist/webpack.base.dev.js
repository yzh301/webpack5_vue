 ...
 * @LastEditTime: 2024-01-10 17:11:55
...
var _require2 = require("unplugin-vue-components/resolvers"),
    ElementPlusResolver = _require2.ElementPlusResolver;

var AutoImport = require("unplugin-auto-import/webpack");

var isDev = process.env.NODE_ENV === "development"; // 是否是开发模式/*  */
...
module.exports = {
  ...
  module: {
    ...
    }, // 匹配.ts文件
    {
      test: /\.ts$/,
      include: [path.resolve(__dirname, "../src")],
      // 只对项目src文件的ts进行loader解析
      use: ["thread-loader", "babel-loader"]
    }, // js文件配置
    {
      test: /.js$/,
      include: [path.resolve(__dirname, "../src")],
      // 只对项目src文件的js进行loader解析
      use: ["thread-loader", "babel-loader"]
    }, {
      test: /\.css$/,
      //匹配所有的 css 文件
      include: [path.resolve(__dirname, "../src")],
      use: [// 开发环境使用style-looader,打包模式抽离css
  ...
  new HtmlWebpackPlugin({
  ...
  }), new WebpackBar(), Components["default"]({
    // 自定义组件的解析器
    ...
    include: [/\.tsx?$/, // 添加对 tsx 文件的支持
    /\.jsx?$/, /\.vue$/, /\.vue\?vue/, /\.md$/],
    // global imports to register
    imports: [// presets
    "vue", "vue-router" // custom
    ],
